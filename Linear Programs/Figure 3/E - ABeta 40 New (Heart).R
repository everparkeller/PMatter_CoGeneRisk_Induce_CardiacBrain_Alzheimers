...
model1.A1 <- lm(as.formula(MF1), data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),])
model1.A2 <- lm(as.formula(MF2), data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),])
model1.A3 <- lm(as.formula(MF3), data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),])
model1.A4 <- lm(as.formula(MF4), data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),])
model1.A5 <- lm(as.formula(MF5), data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),])
model1.A6 <- lm(as.formula(MF6), data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),])
...
small.pmb <- fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),which(colnames(fig.3.data.2) %in% c("Genotype", "Sex", "Exposure", Outcome))]
...
# Group Sex
AD.FA.Max <- max(as.matrix(fig.3.data.2[,Outcome])[which(fig.3.data.2$Exposure == "FA" & fig.3.data.2$Genotype == "AD" & fig.3.data.2$Organ == "Heart")], na.rm = TRUE)
AD.PM.Max <- max(as.matrix(fig.3.data.2[,Outcome])[which(fig.3.data.2$Exposure == "PM" & fig.3.data.2$Genotype == "AD" & fig.3.data.2$Organ == "Heart")], na.rm = TRUE)
WT.FA.Max <- max(as.matrix(fig.3.data.2[,Outcome])[which(fig.3.data.2$Exposure == "FA" & fig.3.data.2$Genotype == "WT" & fig.3.data.2$Organ == "Heart")], na.rm = TRUE)
WT.PM.Max <- max(as.matrix(fig.3.data.2[,Outcome])[which(fig.3.data.2$Exposure == "PM" & fig.3.data.2$Genotype == "WT" & fig.3.data.2$Organ == "Heart")], na.rm = TRUE)

seg.max.1 <- 0.0003
seg.max.2 <- 0.0001
text.above.1 <- 0.00035
text.corner.1 <- 0.0004
text.corner.2 <- 0.0003
ylab.statement <- expression(paste("A", beta, "-40 Integrated Density (signal : total protein)", sep = ""))
yaxis.min <- 0
yaxis.max <- 0.0025
yaxis.breaks <- 0.0005

Figure3A.5 <- ggplot(data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
...
final.model.results <- cbind(c("Intercept", "Genotype", "Exposure"),
                             round(summary(model1.A6)$coef[,1:3], 5),
                             round(confint(model1.A6), 5),
                             ifelse(summary(model1.A6)$coef[,4] < 0.0001, "< 0.0001", round(summary(model1.A6)$coef[,4], 5)))
