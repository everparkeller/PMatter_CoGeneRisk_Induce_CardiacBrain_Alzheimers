#########################################
### Figure 3B - ABeta 40 Norm Sig New ###
#########################################
...
Figure3A.RP <- resid_panel(model1.A2)
...
# Group Sex
fig.3.data.1$Concat.G.S <- ifelse(fig.3.data.1$Genotype == "WT" & fig.3.data.1$Sex == "M", "Male\nWT",
                                  ifelse(fig.3.data.1$Genotype == "WT" & fig.3.data.1$Sex == "F", "Fem\nWT",
                                         ifelse(fig.3.data.1$Genotype == "AD" & fig.3.data.1$Sex == "M", "Male\nAD",
                                                ifelse(fig.3.data.1$Genotype == "AD" & fig.3.data.1$Sex == "F", "Fem\nAD", "Error"))))
...
text.corner.1 <- 1.4
text.corner.2 <- 0.4
ylab.statement <- expression(paste("A", beta, "40 / Total Protein (pg/mg)", sep = ""))
yaxis.min <- 0
yaxis.max <- 25
yaxis.breaks <- 2.5

Figure3A.1 <- ggplot(data = fig.3.data.1[which(fig.3.data.1$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + text.corner.1, label = "*** Sex"), color = "black", size = 4, hjust = 0, vjust = 0) + 
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + text.corner.2, label = "*** Genotype:Sex"), color = "black", size = 4, hjust = 0, vjust = 0)



# Stratified Sex
AD.FA.Max.M <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "FA" & fig.3.data.1$Genotype == "AD" & fig.3.data.1$Sex == "M" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
AD.PM.Max.M <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "PM" & fig.3.data.1$Genotype == "AD" & fig.3.data.1$Sex == "M" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
WT.FA.Max.M <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "FA" & fig.3.data.1$Genotype == "WT" & fig.3.data.1$Sex == "M" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
WT.PM.Max.M <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "PM" & fig.3.data.1$Genotype == "WT" & fig.3.data.1$Sex == "M" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)

AD.FA.Max.F <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "FA" & fig.3.data.1$Genotype == "AD" & fig.3.data.1$Sex == "F" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
AD.PM.Max.F <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "PM" & fig.3.data.1$Genotype == "AD" & fig.3.data.1$Sex == "F" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
WT.FA.Max.F <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "FA" & fig.3.data.1$Genotype == "WT" & fig.3.data.1$Sex == "F" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
WT.PM.Max.F <- max(as.matrix(fig.3.data.1[,Outcome])[which(fig.3.data.1$Exposure == "PM" & fig.3.data.1$Genotype == "WT" & fig.3.data.1$Sex == "F" & fig.3.data.1$Organ == "Heart")], na.rm = TRUE)
...
Figure3A.2 <- ggplot(data = fig.3.data.1[which(fig.3.data.1$Organ == "Heart"),], aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 3) + 
  ...
  theme_bw()  + theme(panel.spacing = unit(0.8, "lines")) +
  theme(plot.title = element_text(size = rel(1.2))) + theme(strip.text.x = element_text()) + 
  xlab("Genotype by Sex") + ylab(ylab.statement) + 
  theme(axis.text = element_text(size = 30, face = "bold")) +
