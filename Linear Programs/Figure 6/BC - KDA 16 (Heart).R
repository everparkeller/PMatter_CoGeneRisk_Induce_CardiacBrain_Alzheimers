...
model1.A1 <- lm(as.formula(MF1), data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),])
model1.A2 <- lm(as.formula(MF2), data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),])
model1.A3 <- lm(as.formula(MF3), data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),])
model1.A4 <- lm(as.formula(MF4), data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),])
model1.A5 <- lm(as.formula(MF5), data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),])
model1.A6 <- lm(as.formula(MF6), data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),])
...
Figure6A.RP <- resid_panel(model1.A5)
...
small.pmb <- fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),which(colnames(fig.6.sod.data.1) %in% c("Genotype", "Sex", "Exposure", Outcome))]
...
# Group Sex
AD.FA.Max <- max(as.matrix(fig.6.sod.data.1[,Outcome])[which(fig.6.sod.data.1$Exposure == "FA" & fig.6.sod.data.1$Genotype == "AD" & fig.6.sod.data.1$Organ == "Heart")], na.rm = TRUE)
AD.PM.Max <- max(as.matrix(fig.6.sod.data.1[,Outcome])[which(fig.6.sod.data.1$Exposure == "PM" & fig.6.sod.data.1$Genotype == "AD" & fig.6.sod.data.1$Organ == "Heart")], na.rm = TRUE)
WT.FA.Max <- max(as.matrix(fig.6.sod.data.1[,Outcome])[which(fig.6.sod.data.1$Exposure == "FA" & fig.6.sod.data.1$Genotype == "WT" & fig.6.sod.data.1$Organ == "Heart")], na.rm = TRUE)
WT.PM.Max <- max(as.matrix(fig.6.sod.data.1[,Outcome])[which(fig.6.sod.data.1$Exposure == "PM" & fig.6.sod.data.1$Genotype == "WT" & fig.6.sod.data.1$Organ == "Heart")], na.rm = TRUE)
...
Figure6A.1 <- ggplot(data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 1.68, y = 1.75, label = "** Exposure"), color = "black", size = 2, hjust = 0, vjust = 0) + 
  geom_text(aes(x = 1.68, y = 1.5, label = "* Genotype:Exposure"), color = "black", size = 2, hjust = 0, vjust = 0) +
  geom_text(aes(x = 1.78, y = 1.9, label = ""))
