...
Figure1A.RP <- resid_panel(model1.A5)
...
Figure1A.11 <- ggplot(data = fig.1.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  scale_y_continuous(breaks = seq(-200, 500, 50)) +
  ...
  xlab("Genotype") + ylab(expression(paste(Delta, " Time in Novel Zone"))) + 
  ...
  geom_segment(aes(x = 1.825, xend = 2.175, y = max(as.matrix(fig.1.data.1[,Outcome]), na.rm = TRUE) + 40, yend = max(as.matrix(fig.1.data.1[,Outcome]), na.rm = TRUE) + 40), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_text(aes(x = 2, y = max(as.matrix(fig.1.data.1[,Outcome]), na.rm = TRUE) + 32, label = "**"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 1.76, y = max(!!sym(Outcome), na.rm = TRUE) + 60, label = ""), color = "black", size = 2, hjust = 0, vjust = 0) 
