###############################################
### Figure 2F - Pulmonary Art Peak Velocity ###
###############################################
...
Figure2A.RP <- resid_panel(model1.A4)
...
Figure2A.1 <- ggplot(data = fig.2.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  scale_y_continuous(breaks = seq(-1000, 0, 100)) +
  ...
  xlab("Genotype") + ylab("Peak Velocity (mm/s)") + 
  ...
  geom_segment(aes(x = 0.824, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 20, yend = max(!!sym(Outcome), na.rm = TRUE) + 20), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 0.825, xend = 0.825, y = max(!!sym(Outcome), na.rm = TRUE) + 20, yend = AD.FA.Max + 10), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.175, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 20, yend = AD.PM.Max + 10), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.824, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 20, yend = max(!!sym(Outcome), na.rm = TRUE) + 20), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 1.825, xend = 1.825, y = max(!!sym(Outcome), na.rm = TRUE) + 20, yend = WT.FA.Max + 10), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 2.175, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 20, yend = WT.PM.Max + 10), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_text(aes(x = 1, y = max(!!sym(Outcome), na.rm = TRUE) + 25, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2, y = max(!!sym(Outcome), na.rm = TRUE) + 25, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 30, label = "** Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) 
...
Figure2A.6 <- ggplot(data = fig.2.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  ...
  geom_text(aes(x = 3.98, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 5, label = "** Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 4.03, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 35, label = ""))
