########################################
### Figure 6H - 20 Minute Incubation ###
########################################
...
Figure6A.12 <- ggplot(data = fig.6.epr.data.1[which(!is.na(fig.6.epr.data.1$Mouse.ID)),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
