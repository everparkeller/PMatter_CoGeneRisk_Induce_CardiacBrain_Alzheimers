...
small.pmb.mean <- mean(as.numeric(as.matrix(small.pmb[,Outcome])), na.rm = TRUE)
small.pmb.sd <- sd(as.numeric(as.matrix(small.pmb[,Outcome])), na.rm = TRUE)
...
# Group Sex
AD.FA.Max <- max(as.numeric(as.matrix(fig.2.data.1[,Outcome]))[which(fig.2.data.1$Exposure == "FA" & fig.2.data.1$Genotype == "AD")], na.rm = TRUE)
AD.PM.Max <- max(as.numeric(as.matrix(fig.2.data.1[,Outcome]))[which(fig.2.data.1$Exposure == "PM" & fig.2.data.1$Genotype == "AD")], na.rm = TRUE)
WT.FA.Max <- max(as.numeric(as.matrix(fig.2.data.1[,Outcome]))[which(fig.2.data.1$Exposure == "FA" & fig.2.data.1$Genotype == "WT")], na.rm = TRUE)
WT.PM.Max <- max(as.numeric(as.matrix(fig.2.data.1[,Outcome]))[which(fig.2.data.1$Exposure == "PM" & fig.2.data.1$Genotype == "WT")], na.rm = TRUE)
...
Figure2A.1 <- ggplot(data = fig.2.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  xlab("Genotype") + ylab("LV Mass : Tibia Length (mg:mm)") + 
  ...
  geom_segment(aes(x = 0.824, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 2, yend = max(!!sym(Outcome), na.rm = TRUE) + 2), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 0.825, xend = 0.825, y = max(!!sym(Outcome), na.rm = TRUE) + 2, yend = AD.FA.Max + 0.7), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.175, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 2, yend = AD.PM.Max + 0.7), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.824, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 2, yend = max(!!sym(Outcome), na.rm = TRUE) + 2), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 1.825, xend = 1.825, y = max(!!sym(Outcome), na.rm = TRUE) + 2, yend = WT.FA.Max + 0.7), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 2.175, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 2, yend = WT.PM.Max + 0.7), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_text(aes(x = 1, y = max(!!sym(Outcome), na.rm = TRUE) + 2.3, label = "#"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2, y = max(!!sym(Outcome), na.rm = TRUE) + 2.3, label = "#"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 3, label = "*** Genotype"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 2.5, label = "** Exposure"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 2, label = "*** Genotype:Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) 
...
Figure2A.2 <- ggplot(data = fig.2.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  ...
  theme(legend.text = element_text(size = 6, face = "bold")) +
  theme(legend.title.align = 0.5) + theme(legend.title = element_text(size = 7, face = "bold")) +
  geom_text(aes(x = 3.16, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 3.3, label = "*** Genotype"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.16, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 2.2, label = "* Exposure"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.16, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 1, label = "*** Genotype:Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.26, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 4, label = ""), color = "black", size = 2.5, hjust = 0, vjust = 0)
