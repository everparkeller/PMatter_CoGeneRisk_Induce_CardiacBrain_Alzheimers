...
Figure4A.5 <- ggplot(data = fig.4a.data.1[which(fig.4a.data.1$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
...
text.corner.1 <- 0.012
text.corner.2 <- 0.008
ylab.statement <- "OC Integrated Density (signal : total protein)"
yaxis.min <- 0
yaxis.max <- 0.5
yaxis.breaks <- 0.02
