################################################
### Figure 6EG (Alt) - MDA/GSH Ratio (Heart) ###
################################################
...
Figure6A.1 <- ggplot(data = fig.6.gsh.data.1[which(fig.6.gsh.data.1$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + text.corner.1, label = "# Exposure"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + text.corner.2, label = "*** Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0)
...
Figure6A.7 <- ggplot(data = fig.6.gsh.data.1[which(fig.6.gsh.data.1$Organ == "Heart"),], aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  ...
  geom_text(aes(x = 3.67, y = 0.3, label = "# Exposure"), color = "black", size = 2, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.67, y = 0.25, label = "*** Sex"), color = "black", size = 2, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.67, y = 0.33, label = ""))
