...
Figure4A.1 <- ggplot(data = fig.4a.data.1[which(fig.4a.data.1$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 2.13, y = max(!!sym(Outcome), na.rm = TRUE) + 2*text.corner.1, label = "* Exposure"), color = "black", size = 2.5, vjust = 0, hjust = 0) +
  geom_text(aes(x = 2.13, y = max(!!sym(Outcome), na.rm = TRUE) + text.corner.1, label = "*** Sex"), color = "black", size = 2.5, vjust = 0, hjust = 0)



# Stratified Sex
fig.4a.data.1$Concat.G.S <- ifelse(fig.4a.data.1$Genotype == "WT" & fig.4a.data.1$Sex == "M", "Male\nWT",
                                   ifelse(fig.4a.data.1$Genotype == "WT" & fig.4a.data.1$Sex == "F", "Female\nWT",
                                          ifelse(fig.4a.data.1$Genotype == "AD" & fig.4a.data.1$Sex == "M", "Male\nAD",
                                                 ifelse(fig.4a.data.1$Genotype == "AD" & fig.4a.data.1$Sex == "F", "Female\nAD", "Error"))))
...
Figure4A.1T <- ggplot(data = fig.4a.data.1[which(fig.4a.data.1$Organ == "Brain"),], aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  geom_boxplot(position = position_dodge(width = 0.7), width = 0.5, size = 0.4, outlier.shape = NA, show.legend = FALSE) + 
  geom_point(aes(fill = factor(Exposure, labels = c("FA1", "PM1"))), position = position_jitterdodge(jitter.width = 0.3, dodge.width = 0.7), color = "black", shape = 21, size = 1, show.legend = FALSE) +
  scale_y_continuous(breaks = seq(yaxis.min, yaxis.max, yaxis.breaks)) +
  ...
  geom_text(aes(x = 3.03, y = max(as.matrix(fig.4a.data.1[which(fig.4a.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + 0.02, label = "* Exposure"), color = "black", size = 2.5, vjust = 0, hjust = 0) +
  geom_text(aes(x = 3.03, y = max(as.matrix(fig.4a.data.1[which(fig.4a.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + 0.01, label = "*** Sex"), color = "black", size = 2.5, vjust = 0, hjust = 0) +
  geom_text(aes(x = 3, y = 0.13, label = ""))
