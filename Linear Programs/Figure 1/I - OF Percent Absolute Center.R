#############################################
### Figure 1I: OF Percent Absolute Center ###
#############################################
...
Figure1A.1 <- ggplot(data = fig.1.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  xlab("Genotype") + ylab("Time in Absolute Center (%)") + 
  ...
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 4.5, label = "** Genotype"), color = "black", size = 2, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 2.5, label = "** Sex"), color = "black", size = 2, hjust = 0, vjust = 0) 
...
Figure1A.7 <- ggplot(data = fig.1.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
