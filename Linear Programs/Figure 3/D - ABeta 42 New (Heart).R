...
text.corner.1 <- 0.08
text.corner.2 <- 0.28
ylab.statement <- expression(paste("A", beta, "-42 Integrated Density (signal : total protein)", sep = ""))
yaxis.min <- 0
yaxis.max <- 1.5
yaxis.breaks <- 0.25

Figure3A.4 <- ggplot(data = fig.3.data.2[which(fig.3.data.2$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
