##########################################
### Figure 2H - MVF MV Flow Decel Time ###
##########################################
...
### Analysis ###
Outcome <- "MVF.MV.Flow.Decel.Time"
...
Pairwise.Comp <- pairs(Est.Marg.Means)
summary(Pairwise.Comp)[c(2,9,24,27),]
...
# Group Sex
AD.FA.Max <- max(as.matrix(fig.2.data.1[,Outcome])[which(fig.2.data.1$Exposure == "FA" & fig.2.data.1$Genotype == "AD")], na.rm = TRUE)
AD.PM.Max <- max(as.matrix(fig.2.data.1[,Outcome])[which(fig.2.data.1$Exposure == "PM" & fig.2.data.1$Genotype == "AD")], na.rm = TRUE)
WT.FA.Max <- max(as.matrix(fig.2.data.1[,Outcome])[which(fig.2.data.1$Exposure == "FA" & fig.2.data.1$Genotype == "WT")], na.rm = TRUE)
WT.PM.Max <- max(as.matrix(fig.2.data.1[,Outcome])[which(fig.2.data.1$Exposure == "PM" & fig.2.data.1$Genotype == "WT")], na.rm = TRUE)
...
Figure2A.1 <- ggplot(data = fig.2.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  xlab("Genotype") + ylab("MV Decel Time (ms)") + 
  ...
  geom_segment(aes(x = 2.175, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 3, yend = WT.PM.Max + 2), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_text(aes(x = 1, y = max(!!sym(Outcome), na.rm = TRUE) + 4, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2, y = max(!!sym(Outcome), na.rm = TRUE) + 4, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 4.5, label = "* Exposure"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 3, label = "** Genotype:Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0)
...
Figure2A.8 <- ggplot(data = fig.2.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  ...
  geom_text(aes(x = 3.22, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 6, label = "* Exposure"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.22, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 3, label = "** Genotype:Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) + 
  geom_text(aes(x = 3.16, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 8, label = ""))
