...
Figure6A.2 <- ggplot(data = fig.6.sod.data.1[which(fig.6.sod.data.1$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 2.09, y = 2.1, label = "* Genotype"), color = "black", size = 2, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.09, y = 2.2, label = ""))
