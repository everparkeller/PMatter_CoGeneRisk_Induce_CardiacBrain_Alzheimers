#################################################
### Figure 3J - ABeta 40 Norm Sig New (Brain) ###
#################################################
...
Figure3A.RP <- resid_panel(model1.A3)



# Test Pairwise Comparisons (Adjusted for Multiple Comparisons)
Est.Marg.Means <- emmeans(model1.A3, ~ Genotype + Exposure)
Pairwise.Comp <- pairs(Est.Marg.Means)
summary(Pairwise.Comp)

Est.Marg.Means <- emmeans(model1.A3, ~ Genotype + Exposure + Sex)
Pairwise.Comp <- pairs(Est.Marg.Means)
summary(Pairwise.Comp)
...
Figure3A.1 <- ggplot(data = fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_segment(aes(x = 2.175, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + seg.max.1, yend = WT.PM.Max + seg.max.2), 
               color = "black", size = 3, lineend = "round") + 
  geom_text(aes(x = 1, y = max(!!sym(Outcome), na.rm = TRUE) + text.above.1, label = "*"), color = "black", size = 4, vjust = 0) +
  ...
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) - 0.4, label = "** Sex"), color = "black", size = 4, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) - 1.3, label = "** Genotype:Exposure"), color = "black", size = 4, hjust = 0, vjust = 0)
...
Figure3A.8 <- ggplot(data = fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),], aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 3) + 
  ...
  geom_segment(aes(x = 0.825, xend = 1.175, y = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 2.5, lineend = "round") +
  geom_text(aes(x = 1, y = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + text.above.1 + 0.2, label = "#"), color = "black", size = 8, vjust = 0) +
  geom_segment(aes(x = 2.825, xend = 3.175, y = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 2.5, lineend = "round") +
  geom_text(aes(x = 3, y = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + text.above.1 + 0.2, label = "#"), color = "black", size = 8, vjust = 0) +
  geom_text(aes(x = 3, y = max(as.matrix(fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),Outcome]), na.rm = TRUE) + text.above.1 + 0.5, label = ""))



# Create Final Model
final.model.results <- cbind(c("Intercept", "Genotype", "Exposure", "Sex", "Genotype:Exposure"),
