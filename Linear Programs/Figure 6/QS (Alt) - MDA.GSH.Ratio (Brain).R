################################################
### Figure 6QS (Alt) - MDA/GSH Ratio (Brain) ###
################################################
...
seg.max.2 <- 0.001
text.above.1 <- 0.0025
text.corner.1 <- 0.0026
text.corner.2 <- 0.0006
ylab.statement <- "MDA / GSH Ratio"
yaxis.min <- 0
yaxis.max <- 1
yaxis.breaks <- 0.01

Figure6A.11 <- ggplot(data = fig.6.gsh.data.1[which(fig.6.gsh.data.1$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
