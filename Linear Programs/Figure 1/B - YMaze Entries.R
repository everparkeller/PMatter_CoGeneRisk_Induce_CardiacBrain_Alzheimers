...
Figure1A.1 <- ggplot(data = fig.1.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  xlab("Genotype") + ylab("Arm Entries (#)") + 
  ...
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome)) + 1.5, label = "** Sex"), color = "black", size = 2, hjust = 0, vjust = 0)
...
Figure1A.2 <- ggplot(data = fig.1.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
