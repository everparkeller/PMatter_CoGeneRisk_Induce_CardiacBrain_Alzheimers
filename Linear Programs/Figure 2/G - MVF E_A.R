...
Figure2A.1 <- ggplot(data = fig.2.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_segment(aes(x = 0.824, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.4, yend = max(!!sym(Outcome), na.rm = TRUE) + 0.4), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 0.825, xend = 0.825, y = max(!!sym(Outcome), na.rm = TRUE) + 0.4, yend = AD.FA.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.175, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.4, yend = AD.PM.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.824, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.4, yend = max(!!sym(Outcome), na.rm = TRUE) + 0.4), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 1.825, xend = 1.825, y = max(!!sym(Outcome), na.rm = TRUE) + 0.4, yend = WT.FA.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 2.175, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.4, yend = WT.PM.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_text(aes(x = 1, y = max(!!sym(Outcome), na.rm = TRUE) + 0.45, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2, y = max(!!sym(Outcome), na.rm = TRUE) + 0.45, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 0.6, label = "* Genotype"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, label = "* Genotype:Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) 
...
Figure2A.7 <- ggplot(data = fig.2.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  ...
  geom_text(aes(x = 3.26, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 0.3, label = "* Genotype"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.26, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 0.1, label = "* Genotype:Sex"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.26, y = max(as.matrix(fig.2.data.1[,Outcome]), na.rm = TRUE) + 0.4, label = ""))
