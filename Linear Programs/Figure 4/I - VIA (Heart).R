...
Figure4A.6 <- ggplot(data = fig.4a.data.1[which(fig.4a.data.1$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 1.73, y = 0.27, label = "# Genotype"), color = "black", size = 2.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2, y = 0.28, label = ""))
