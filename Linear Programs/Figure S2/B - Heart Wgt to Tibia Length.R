#################################################
### Figure S2B - Heart Weight to Tibia Length ###
#################################################
...
text.corner.1 <- 2
text.corner.2 <- 1.5
ylab.statement <- "Heart Weight : Tibia Length (mg : mm)"
yaxis.min <- 0
yaxis.max <- 50
yaxis.breaks <- 1

Figure6A.1 <- ggplot(data = fig.s2.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.8) + 
  geom_boxplot(position = position_dodge(width = 0.7), width = 0.5, size = 0.8, outlier.shape = NA) + 
...
# Stratified Sex
fig.s2.data.1$Concat.G.S <- ifelse(fig.s2.data.1$Genotype == "WT" & fig.s2.data.1$Sex == "M", "Male\nWT",
                                   ifelse(fig.s2.data.1$Genotype == "WT" & fig.s2.data.1$Sex == "F", "Female\nWT",
                                          ifelse(fig.s2.data.1$Genotype == "AD" & fig.s2.data.1$Sex == "M", "Male\nAD",
                                                 ifelse(fig.s2.data.1$Genotype == "AD" & fig.s2.data.1$Sex == "F", "Female\nAD", "Error"))))

AD.FA.Max.M <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "FA" & fig.s2.data.1$Genotype == "AD" & fig.s2.data.1$Sex == "M")], na.rm = TRUE)
AD.PM.Max.M <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "PM" & fig.s2.data.1$Genotype == "AD" & fig.s2.data.1$Sex == "M")], na.rm = TRUE)
WT.FA.Max.M <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "FA" & fig.s2.data.1$Genotype == "WT" & fig.s2.data.1$Sex == "M")], na.rm = TRUE)
WT.PM.Max.M <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "PM" & fig.s2.data.1$Genotype == "WT" & fig.s2.data.1$Sex == "M")], na.rm = TRUE)

AD.FA.Max.F <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "FA" & fig.s2.data.1$Genotype == "AD" & fig.s2.data.1$Sex == "F")], na.rm = TRUE)
AD.PM.Max.F <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "PM" & fig.s2.data.1$Genotype == "AD" & fig.s2.data.1$Sex == "F")], na.rm = TRUE)
WT.FA.Max.F <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "FA" & fig.s2.data.1$Genotype == "WT" & fig.s2.data.1$Sex == "F")], na.rm = TRUE)
WT.PM.Max.F <- max(as.matrix(fig.s2.data.1[,Outcome])[which(fig.s2.data.1$Exposure == "PM" & fig.s2.data.1$Genotype == "WT" & fig.s2.data.1$Sex == "F")], na.rm = TRUE)
...
FigureS2A.2 <- ggplot(data = fig.s2.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.8) + 
  ...
  geom_text(aes(x = 3.33, y = 13, label = "* Genotype"), color = "black", size = 4, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.33, y = 12.5, label = "** Genotype:Sex"), color = "black", size = 4, hjust = 0, vjust = 0) +
  geom_text(aes(x = 3.33, y = 13.2, label = ""))
