#################################################
### Figure 3F - ABeta 42 Norm Sig New (Brain) ###
#################################################
...
text.corner.1 <- 1.4
text.corner.2 <- 0.4
ylab.statement <- expression(paste("A", beta, "42 / Total Protein (pg/mg)", sep = ""))
yaxis.min <- 0
yaxis.max <- 15
yaxis.breaks <- 2

Figure3A.7 <- ggplot(data = fig.3.data.1[which(fig.3.data.1$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  theme(axis.title = element_text(size = 29, face = "bold")) +
