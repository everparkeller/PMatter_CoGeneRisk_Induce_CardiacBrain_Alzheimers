...
# Group Sex
AD.FA.Max <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "FA" & fig.1.data.1$Genotype == "AD")], na.rm = TRUE)
AD.PM.Max <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "PM" & fig.1.data.1$Genotype == "AD")], na.rm = TRUE)
WT.FA.Max <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "FA" & fig.1.data.1$Genotype == "WT")], na.rm = TRUE)
WT.PM.Max <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "PM" & fig.1.data.1$Genotype == "WT")], na.rm = TRUE)
...
Figure1A.1 <- ggplot(data = fig.1.data.1, aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  xlab("Genotype") + ylab("Distance in Open Arm (m)") + 
  ...
  geom_segment(aes(x = 0.824, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, yend = max(!!sym(Outcome), na.rm = TRUE) + 0.5), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 0.825, xend = 0.825, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, yend = AD.FA.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.175, xend = 1.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, yend = AD.PM.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 1.824, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, yend = max(!!sym(Outcome), na.rm = TRUE) + 0.5), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(aes(x = 1.825, xend = 1.825, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, yend = WT.FA.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(aes(x = 2.175, xend = 2.175, y = max(!!sym(Outcome), na.rm = TRUE) + 0.5, yend = WT.PM.Max + 0.2), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_text(aes(x = 1, y = max(!!sym(Outcome), na.rm = TRUE) + 0.6, label = "NS"), color = "black", size = 2, vjust = 0) +
  geom_text(aes(x = 2, y = max(!!sym(Outcome), na.rm = TRUE) + 0.6, label = "NS"), color = "black", size = 2, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 0.65, label = "* Sex"), color = "black", size = 2, hjust = 0, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + 0.25, label = "# Exposure"), color = "black", size = 2, hjust = 0, vjust = 0) 



# Stratified Sex
AD.FA.Max.M <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "FA" & fig.1.data.1$Genotype == "AD" & fig.1.data.1$Sex == "M")], na.rm = TRUE)
AD.PM.Max.M <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "PM" & fig.1.data.1$Genotype == "AD" & fig.1.data.1$Sex == "M")], na.rm = TRUE)
WT.FA.Max.M <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "FA" & fig.1.data.1$Genotype == "WT" & fig.1.data.1$Sex == "M")], na.rm = TRUE)
WT.PM.Max.M <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "PM" & fig.1.data.1$Genotype == "WT" & fig.1.data.1$Sex == "M")], na.rm = TRUE)

AD.FA.Max.F <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "FA" & fig.1.data.1$Genotype == "AD" & fig.1.data.1$Sex == "F")], na.rm = TRUE)
AD.PM.Max.F <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "PM" & fig.1.data.1$Genotype == "AD" & fig.1.data.1$Sex == "F")], na.rm = TRUE)
WT.FA.Max.F <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "FA" & fig.1.data.1$Genotype == "WT" & fig.1.data.1$Sex == "F")], na.rm = TRUE)
WT.PM.Max.F <- max(as.matrix(fig.1.data.1[,Outcome])[which(fig.1.data.1$Exposure == "PM" & fig.1.data.1$Genotype == "WT" & fig.1.data.1$Sex == "F")], na.rm = TRUE)

Figure1A.6 <- ggplot(data = fig.1.data.1, aes(x = Concat.G.S, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
