...
F2.Table <- data.frame(Predictors = c("Intercept", "Genotype", "Exposure", "Genotype:Exposure"),
...
text.corner.1 <- 250
text.corner.2 <- 220 
ylab.statement <- expression(paste("ROS Level (Intensity per   ", mu, "g / ml protein)"))
yaxis.min <- 0
yaxis.max <- 4000
yaxis.breaks <- 500

Figure6A.13 <- ggplot(data = fig.6.epr.data.1[which(!is.na(fig.6.epr.data.1$Mouse.ID)),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 1.68, y = 2800, label = "# Exposure"), color = "black", size = 1.5, hjust = 0, vjust = 0) +
  geom_text(aes(x = 1.68, y = 2900, label = ""))
