####################################################
### Figure 3N - Ratio ABeta 42 to 40 New (Brain) ###
####################################################
...
Figure3A.12 <- ggplot(data = fig.3.data.2[which(fig.3.data.2$Organ == "Brain"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  ...
  geom_text(aes(x = 1, y = max(as.matrix(fig.3.data.2[which(fig.3.data.2$Organ == "Brain"),Outcome]), na.rm = TRUE) + text.above.1 + 0.5, label = "#"), color = "black", size = 8, vjust = 0) +
  geom_segment(aes(x = 1.825, xend = 2.175, y = max(as.matrix(fig.3.data.2[which(fig.3.data.2$Organ == "Brain"),Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.3.data.2[which(fig.3.data.2$Organ == "Brain"),Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 2.5, lineend = "round") +
  geom_text(aes(x = 2, y = max(as.matrix(fig.3.data.2[which(fig.3.data.2$Organ == "Brain"),Outcome]), na.rm = TRUE) + text.above.1 + 0.5, label = "#"), color = "black", size = 8, vjust = 0) +
  geom_text(aes(x = 1, y = 141, label = ""))
...
final.model.results <- cbind(c("Intercept", "Genotype", "Exposure"),
                             ...
                             ifelse(summary(model1.A6)$coef[,4] < 0.0001, "< 0.0001", round(summary(model1.A6)$coef[,4], 4)))
