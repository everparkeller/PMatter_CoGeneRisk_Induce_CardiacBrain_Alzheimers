####################################################
### Figure 6DF (Alt) - MDA Concentration (Heart) ###
####################################################
...
text.corner.1 <- 0.26
text.corner.2 <- 0.17
ylab.statement <- expression(paste("MDA Concentration ( ", mu, "mol / ", mu, "g)"))
yaxis.min <- 0
yaxis.max <- 3
yaxis.breaks <- 0.2

Figure6A.6 <- ggplot(data = fig.6.gsh.data.1[which(fig.6.gsh.data.1$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
...
Figure2A.2 <- ggplot(data = fig.6.gsh.data.1[which(fig.6.gsh.data.1$Organ == "Heart"),], aes(x = Genotype, y = !!sym(Outcome), fill = Exposure)) +
  stat_boxplot(position = position_dodge(width = 0.7), geom = "errorbar", width = 0.2, size = 0.4) + 
  ...
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "F"),
               aes(x = 0.824, xend = 1.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "F"),
               aes(x = 0.825, xend = 0.825, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = AD.FA.Max.F + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "F"),
               aes(x = 1.175, xend = 1.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = AD.PM.Max.F + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "F"),
               aes(x = 1.824, xend = 2.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "F"),
               aes(x = 1.825, xend = 1.825, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = WT.FA.Max.F + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "F"),
               aes(x = 2.175, xend = 2.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = WT.PM.Max.F + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "M"),
               aes(x = 0.824, xend = 1.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "M"),
               aes(x = 0.825, xend = 0.825, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = AD.FA.Max.M + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "M"),
               aes(x = 1.175, xend = 1.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = AD.PM.Max.M + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "M"),
               aes(x = 1.824, xend = 2.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1), 
               color = "black", size = 0.4, lineend = "round") + 
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "M"),
               aes(x = 1.825, xend = 1.825, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = WT.FA.Max.M + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_segment(data = subset(fig.6.gsh.data.1, Sex == "M"),
               aes(x = 2.175, xend = 2.175, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + seg.max.1, yend = WT.PM.Max.M + seg.max.2), 
               color = "black", size = 0.4, lineend = "round") +
  geom_text(data = subset(fig.6.gsh.data.1, Sex == "F"),
            aes(x = 1, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + text.above.1, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(data = subset(fig.6.gsh.data.1, Sex == "F"),
            aes(x = 2, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + text.above.1, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(data = subset(fig.6.gsh.data.1, Sex == "M"),
            aes(x = 1, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + text.above.1, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(data = subset(fig.6.gsh.data.1, Sex == "M"),
            aes(x = 2, y = max(as.matrix(fig.6.gsh.data.1[,Outcome]), na.rm = TRUE) + text.above.1, label = "NS"), color = "black", size = 2.5, vjust = 0) +
  geom_text(aes(x = 2.32, y = max(!!sym(Outcome), na.rm = TRUE) + text.corner.1, label = "# Sex"), color = "black", size = 2.5, vjust = 0)
